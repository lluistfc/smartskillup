...
function M.new(ctx)
    ...
    local function start()
        ctx.refresh_catalog();
        if (#actions() == 0) then return false, 'Select at least one currently available action first.'; end
        state.resting = false; state.skill_cursor = 1;
    ...
    local function tick()
        ...
        if (state.resting) then
            if (mpp >= state.settings.resume_above[1]) then
                ctx.queue('/heal'); state.resting = false; state.status = 'Running'; state.next_action = ctx.now() + 1;
            else state.status = ('Resting (%d%% MP)'):fmt(mpp); state.next_action = ctx.now() + 1; end
            return;
        elseif (state.settings.rest_below[1] > 0 and mpp <= state.settings.rest_below[1]) then
            ctx.queue('/heal'); state.resting = true; state.status = ('Resting (%d%% MP)'):fmt(mpp);
            state.next_action = ctx.now() + 2; return;
        end
        local selected = actions();
        if (#selected == 0) then ctx.stop('Stopped: no actions selected.'); return; end
