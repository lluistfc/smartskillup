...
function M.new(ctx)
    ...
    local function tick()
        if ctx.player_engaged() then
            state.status = 'AFK: engaged; auto-target will chain attackers'
            return
        end
        ...
        ctx.queue '/attack <t>'
        afk.last_target = entity.Name or 'Unknown'
    ...
    local function on_packet(e)
        ...
        if player_id == 0 or actor == player_id then
            return
        end
        for _, target in ipairs(targets) do
            if target == player_id then
                ...
                if hostile(index) then
                    ...
                    afk.last_target = GetEntity(index).Name or 'Unknown'
                    state.status = 'AFK: attacker detected ' .. afk.last_target
                end
                return
            end
        end
